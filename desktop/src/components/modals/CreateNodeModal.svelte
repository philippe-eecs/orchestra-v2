<script lang="ts">
  ...
  import type { NodeCreate, NodeStatus, AgentType, Resource } from '../../lib/types';
  import { createNode, graphError } from '../../stores/graph';
  ...
  async function handleSubmit() {
    ...
    if (result) {
      ...
      error = $graphError || 'Failed to create node';
    }
    saving = false;
  }
</script>

<Modal title="Create Node" {open} on:close={handleClose} fullscreen>
  <form on:submit|preventDefault={handleSubmit}>
    <div class="layout">
      <div class="main-section">
        <div class="field">
          <label for="title">Title *</label>
          <input id="title" bind:value={title} placeholder="What needs to be done?" />
        </div>
...
Example:
- Background: We need to improve API performance
- Analyze the current database queries
- Identify N+1 query problems
- Propose specific optimizations with code examples"
          ></textarea>
        </div>
...
- Working implementation of feature X
...
- Updated documentation"
      ...
      <div class="side-section">
        ...
        <div class="field">
          ...
          <select id="agent" bind:value={agentType}>
